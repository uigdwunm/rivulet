...
public class MysqlAssemblyLine implements AssemblyLine {
     ...
     * 如果没查到说明没有初始化过
     **/
    private final Map<Blueprint, Function<ParamManager, SqlAssemblyLinePortableToolbox>> toolboxCreatorMap = new ConcurrentHashMap<>();
    ...
    public void warmUp(Blueprint blueprint) {
        ...
        Function<ParamManager, SqlAssemblyLinePortableToolbox> toolboxCreator = this.warmUpAndToolbox(sqlBlueprint);
        toolboxCreatorMap.put(sqlBlueprint, toolboxCreator);
    }

    private Function<ParamManager, SqlAssemblyLinePortableToolbox> warmUpAndToolbox(SQLBlueprint sqlBlueprint) {
        RelationSwitch rootSwitch = RelationSwitch.createRootSwitch();
        SQLAliasManager aliasManager = sqlBlueprint.getAliasManager();
        SqlAssemblyLineWarmUpToolbox sqlAssemblyLineWarmUpToolbox = new SqlAssemblyLineWarmUpToolbox(aliasManager);
        sqlStatementFactory.init(sqlBlueprint, rootSwitch, sqlAssemblyLineWarmUpToolbox);
        return paramManager -> new SqlAssemblyLinePortableToolbox(paramManager, sqlBlueprint.getAliasManager());
    }
    ...
    public Fish generate(Blueprint blueprint, ParamManager paramManager) {
        ...
        Function<ParamManager, SqlAssemblyLinePortableToolbox> toolboxCreator = toolboxCreatorMap.get(blueprint);
        ...
        SqlAssemblyLinePortableToolbox toolbox = toolboxCreator.apply(paramManager);
